/******************************************************************************
 *  Purpose: Stock information program
 *  
 *  @author  Akshay
 *  @version 1.0
 *  @since   20-09-2019
...
public class StockAcount {
	...
	public static void main(String[] args) throws JsonParseException, JsonMappingException, IOException {
		...
		String pathOfTransaction = "/home/admin/seleninum/eclipse-workspace/JavaPrograms/src/com/bridgelabz/jsonfiles/transactions.json";
		...
		for (int i = 0; i < custList.size(); i++) {
			System.out.println("ID "+(i+1)+" for "+custList.get(i).getCustomer_name());
		}
		customerId = Utility.scanInt();
		for (int i = 0; i < custList.size(); i++) {
			
			if (customerId == custList.get(i).getCustomer_symbol()) {
				custName = custList.get(i).getCustomer_name();
		...
		if (isCustomerFound) {
			while (!isExit) {
				...
				case 1:
					...
					System.out.println("Please select company symbol: ");
					
					for (int i = 0; i < compList.size(); i++) {
						System.out.println("company symbol : "+compList.get(i).getCompany_symbol()+" company name "+compList.get(i).getCompany_name());
					}
					...
					for (int i = 0; i < compList.size(); i++) {
						if (companySymbol == compList.get(i).getCompany_symbol()) {
							isCompanyFound = true;
							indexOfCompany = i;
							compName = compList.get(i).getCompany_name();
							compShar = compList.get(i).getCompany_shares();
							break;
						}
					}
					...
					if (isCompanyFound) {
						System.out.println("company name "+compName+" shares of company "+compShar);
						...
						if (numOfCompanyShareToBuy < compList.get(indexOfCompany).getCompany_shares()) {
							...
							if (customerBalance > (numOfCompanyShareToBuy* compList.get(indexOfCompany).getCompany_share_price())) {
								...
								compList.get(indexOfCompany).setCompany_shares(compList.get(indexOfCompany).getCompany_shares() - numOfCompanyShareToBuy);
							
								// calculating new total value of company
								compList.get(indexOfCompany).setCompany_total_value(compList.get(indexOfCompany).getCompany_share_price()*compList.get(indexOfCompany).getCompany_shares());

								// code reflecting transaction
								Transactions trans = new Transactions();
								LocalDateTime now = LocalDateTime.now();

								String transId = dateTimeFormatter2.format(now)+ custList.get(indexOfCustomer).getCustomer_symbol()+ compList.get(indexOfCompany).getCompany_symbol();
				...
				case 2:
					...
					for (int i = 0; i < compList.size(); i++) {
						if(custId==i)
						System.out.println("Hello "+custList.get(i).getCustomer_name()+" you have "+custList.get(i).getCustomer_shares()+" shares");
					}
					...
					for (int i = 0; i < compList.size(); i++) {
						System.out.println("company symbol is "+compList.get(i).getCompany_symbol()+" and  company name "+compList.get(i).getCompany_name());
					}
					companySymbol = Utility.scanInt();
					System.out.println("Validating...");
					...
					if (isCompanyFound2) {
						...
						if (share <= custList.get(indexOfCustomer).getCustomer_shares()) {
							compList.get(indexOfCompany).setCompany_shares(compList.get(indexOfCompany).getCompany_shares() + share);
							...
							custList.get(indexOfCustomer).setCustomer_balance(custList.get(indexOfCustomer).getCustomer_balance() + amountToGet);
